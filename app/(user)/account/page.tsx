...
export default async function AccountPage() {
  ...
  if (!session) { return null; } // NOTE: redirecting is already handled by proxy.

  const user = await user_getInfoById(session.user.id);
  if (!user) { return null; } // NOTE: There must be something wrong if user is null.
    ...
    <main>
      ...
      <ul className="grid w-full max-w-xl gap-3 mx-auto mt-8 px-4">
        ...
        <li>
          ...
          >
            <div className="flex flex-col">
              <span className="text-sm font-medium">Viewed Recently</span>
